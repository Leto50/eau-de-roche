...
export const removeIngredient = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const [
      ...
      ctx.db
        .query("recipes")
        .filter((query) => query.eq(query.field("productId"), product._id))
        .first(),
      ctx.db
      ...
      ctx.db
        .query("transactions")
        .withIndex("by_product_and_date", (index) =>
    ...
    if (producedByRecipe || recipeIngredient) {
      throw new ConvexError({
        code: "IN_USE",
        message:
          "Cet ingrédient est utilisé dans une recette. Retirez-le d’abord des recettes concernées.",
      })
    }
    if (bundleItem) {
      throw new ConvexError({
        code: "IN_USE",
        message:
          "Cet ingrédient est utilisé dans un lot. Retirez-le d’abord des lots concernés.",
      })
    }
    if (orderLine) {
      throw new ConvexError({
        code: "IN_USE",
        message:
          "Cet ingrédient est utilisé dans une commande. Supprimez d’abord les commandes liées.",
      })
    }
    if (transaction || transactionLine || stockMovement) {
      throw new ConvexError({
        code: "IN_USE",
        message:
          "Cet ingrédient possède un historique d’opérations. Supprimez d’abord les opérations liées.",
      })
    }
